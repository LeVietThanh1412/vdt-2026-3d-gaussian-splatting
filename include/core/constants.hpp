...
namespace constants {
    ...
    // Chỉ giữ lại những hạt Gaussian có độ đục (opacity) từ 10% trở lên.
    constexpr float  DEFAULT_MAX_SCALE         = 10.0f;  // Giới hạn tỷ lệ tối đa
    // Loại bỏ những hạt Gaussian bị kéo giãn quá mức
    constexpr int    DEFAULT_POISSON_DEPTH     = 9;      // Độ sâu Poisson mặc định
    // Quyết định độ phân giải (độ chi tiết) của bề mặt lưới khi dùng thuật toán Poisson Surface Reconstruction
    constexpr size_t DEFAULT_TARGET_TRIANGLES  = 50000;  // Số tam giác mục tiêu
    // Ép mô hình sau khi tạo xong phải rút gọn xuống chỉ còn tối đa 50.000 mặt tam giác.
    constexpr float  DEFAULT_NAV_SLOPE_DEG     = 45.0f;  // Góc nghiêng tối đa cho lưới điều hướng
    // Bất kỳ bề mặt nào có độ dốc nghiêng dưới 45 độ so với mặt đất sẽ được coi là "Đường đi"
